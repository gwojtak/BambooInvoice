...
?>
	...
	?>
		...
		<p id="dateIssuedContainer">
			<label><?php echo $this->lang->line('invoice_date_issued');?> <input type="text" name="dateIssued" id="dateIssued" value="<?php echo (set_value('dateIssued')) ? (set_value('dateIssued')) : ($row->dateIssued);?>"/></label>
			<span id="dateIssuedDisplay"><?php echo date('F d, Y', mysql_to_unix((set_value('dateIssued')) ? (set_value('dateIssued')) : ($row->dateIssued)));?></span> <a href="#" id="changeDate" onclick="createInvoiceDate.toggle()"><?php echo $this->lang->line('actions_change');?></a>
		</p>
		...
		<p>
			<label><?php echo $this->lang->line('invoice_note');?> <?php echo form_error('invoice_note'); ?><br />
			<textarea name="invoice_note" id="invoice_note" cols="80" rows="3"><?php echo (set_value('invoice_note')) ? (set_value('invoice_note')) : ($row->invoice_note);?></textarea>
			</label>
		</p>
